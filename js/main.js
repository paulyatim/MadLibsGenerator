...
let cards = document.querySelectorAll(".catalogueCard");

for (let card of cards) {
    card.addEventListener("click", playMadLib);
}
...
function playMadLib(e) {
    ...
    let list = document.getElementById("inputWords");
    list.innerHTML = ``;
    for (const type of card.prompts) {
        inputWords(type);
    }
    let buttonSubmit = document.createElement("div");
    buttonSubmit.innerHTML = `<button onclick="loadWords(${card.id})">Submit Words</button>`;
...
// función que muestra el input para un tipo de palabra específico
function inputWords(type) {
    let field = document.createElement("div");
    field.innerHTML = `${type.toUpperCase()}<input type="text">`;
    document.getElementById("inputWords").appendChild(field);
}
...
function showMadLib(madLibId, answers) {
    ...
    switch (madLibId) {
        case 1:
            text.innerHTML = `<p class="resultTitle">Braveheart MadLibs!</p>
            ...
            break;
        case 2:
            text.innerHTML = `<p class="resultTitle">Game of Thrones MadLibs!</p>
            ...
            break;
        case 3:
            text.innerHTML = `<p class="resultTitle">¡MadLibs en Español!</p>
            ...
            break;
        case 4:
            text.innerHTML = `<p class="resultTitle">Avatar: The Last Airbender MadLibs!</p>
